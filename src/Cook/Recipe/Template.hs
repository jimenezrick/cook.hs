...
useTemplate :: forall a. (Default a, Data a, Typeable a, Generic a, FromJSON a)
...
useTemplateNoDef :: forall a. (Data a, Typeable a, Generic a, FromJSON a)
                 => TemplateConf a -> FilePath -> IO ()
useTemplateNoDef src dst = loadConfNoDef src >>= hastacheFile conf src' >>= Text.writeFile dst
  where conf = hastacheConf $ takeDirectory src'
        src' = untag src

useTemplateWithDef :: forall a. (Data a, Typeable a, Generic a, FromJSON a)
